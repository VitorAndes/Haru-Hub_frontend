...
export function Carousel({ game }: CarouselProps) {
  const { header_image, genres, name, screenshots, short_description } = game;
  ...
  return (
    <div className="flex w-full flex-shrink-0 flex-col gap-2 lg:flex-row">
      <figure className="flex-1 relative">
        ...
        <div className="flex flex-wrap gap-1 absolute right-3 bottom-3 font-bold">
          {visibleGenres.map((genre) => (
            <span
              key={genre.id}
              className="px-3 py-1 bg-backgroundPrimary/90 border border-secondary rounded-lg text-sm text-text/90"
            >
              {genre.description}
            </span>
          ))}
      ...
      <div className="flex flex-col flex-1 gap-4 max-w-96">
        <div className="w-full p-3 lg:bg-primary/25 lg:backdrop-blur lg:shadow-md lg:shadow-secondary rounded-lg lg:overflow-hidden">
